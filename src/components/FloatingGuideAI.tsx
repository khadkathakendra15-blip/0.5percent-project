...
function Lotus({ size = 28 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 40 40" style={{ flexShrink: 0 }}>
      <circle cx="20" cy="20" r="18" fill="rgba(12,26,50,0.12)" stroke={W.gold} strokeWidth="1"/>
      <circle cx="20" cy="20" r="6"  fill="none" stroke={W.gold} strokeWidth="0.6" opacity="0.6"/>
      <path d="M20 6 Q25 14 20 20 Q15 14 20 6" fill="rgba(12,26,50,0.18)" stroke={W.gold} strokeWidth="0.5"/>
      <path d="M34 20 Q26 25 20 20 Q26 15 34 20" fill="rgba(12,26,50,0.18)" stroke={W.gold} strokeWidth="0.5"/>
      <path d="M20 34 Q15 26 20 20 Q25 26 20 34" fill="rgba(12,26,50,0.18)" stroke={W.gold} strokeWidth="0.5"/>
      <path d="M6 20 Q14 15 20 20 Q14 25 6 20" fill="rgba(12,26,50,0.18)" stroke={W.gold} strokeWidth="0.5"/>
    </svg>
  );
}
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:'6px', alignItems: isAI ? 'flex-start' : 'flex-end' }}>
      ...
      <div style={{ display:'flex', alignItems:'flex-end', gap:'8px', maxWidth:'88%', flexDirection: isAI ? 'row' : 'row-reverse' }}>
        ...
        <div style={{
          ...
          padding:'11px 14px',
          borderLeft: isAI ? `2px solid rgba(12,26,50,0.28)` : 'none',
        }}>
          {isAI && (
        ...
        <div style={{ display:'flex', flexWrap:'wrap', gap:'5px', paddingLeft:'34px', maxWidth:'320px' }}>
          {msg.chips.map(c => (
            <button key={c} onClick={() => onOption(c)} style={{
              ...
              background:'rgba(12,26,50,0.08)', border:`1px solid rgba(12,26,50,0.25)`,
              borderRadius:'14px', padding:'5px 12px', cursor:'pointer', transition:'all 0.18s', whiteSpace:'nowrap',
            }}
            onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.background = 'rgba(12,26,50,0.15)'; (e.currentTarget as HTMLButtonElement).style.borderColor = W.gold; }}
            onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.background = 'rgba(12,26,50,0.08)'; (e.currentTarget as HTMLButtonElement).style.borderColor = 'rgba(12,26,50,0.25)'; }}
            >{c}</button>
          ))}
        </div>
      )}
